...
strVarCity += "                        <div id=\"\" style=\"width: 800px; position:relative;\">";
strVarCity += "                          <div class=\"data-result\"><em>最多选择 <strong>20<\/strong> 项<\/em><\/div>";
strVarCity += "                          <div class=\"data-error\" style=\"display: none;\">最多只能选择 20 项<\/div>";
...
//      strVarCity += "                              <\/div>";
        ...
        for (var i in dataCityxz) {
            strVarCity += '<li><a href=\"javascript:;\" data-code=\"' + dataCityxz[i] + '\" data-name=\"' + dataarrary[dataCityxz[i]].name + '\" class=\"d-item\"  onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[dataCityxz[i]].name + '<label>0</label></a></li>';
        }
        strVarCity += "                                <\/ul>";
        strVarCity += "                              <\/div>";
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            var nowcode = relations[$(con).data("code")];
            //添加标题
            if (isremove != "remove") {
            ...
            for (var i in nowcode) {
                strVarCity += '<li><a href="javascript:;" class="d-item" data-code="' + nowcode[i] + '"  data-name=\"' + dataarrary[nowcode[i]].name + '\" onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[nowcode[i]].name+dataarrary[nowcode[i]].suffix + '<label>0</label></a></li>';
            }
            strVarCity += "<\/ul>";
            $('.data-container-city').html(strVarCity);
        } else {
...
function getSelectArea(){
    ...
    var list = [];
    var data = $(dataCityinput).data("value");
    if(isNotBlank(data)){
        var code = data.split("-");
        for(var i = 0;i<code.length;i++){
            var area = dataarrary[code[i]];
            var id = area.id;

...
public class StoreSystemWebMain extends AbstractServer {

	private final static Logger logger = LoggerFactory.getLogger(StoreSystemWebMain.class);

	public StoreSystemWebMain(String[] anArgs) {
		super(anArgs);
	}
	...
	public static void runServer(String... args) throws Exception {
        ...
        StoreSystemWebMain server = new StoreSystemWebMain(args);
		server.run(args);
	}
	...
	public static void main(String[] args) {
		...
		try {
			if (args == null || args.length < 2) {
				...
				args = new String[] { "StoreSystemWebMain", "30005", webapp.toString() };
			}
			runServer(args);
		} catch (Exception e) {
			e.printStackTrace();
